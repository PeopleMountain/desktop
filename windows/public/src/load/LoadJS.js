  ...
  function loadResourceMobile(arg){
        if(typeof arg == "undefined"){
            arg = RESOURCES;
        }
        for(let val in arg){
            if(typeof arg[val].key == "undefined"){
                loadResourceMobile(arg[val]);
            } else {
                if(typeof arg[val].path != "undefined"){
                     dynamicLoadJs(arg[val].path,()=>{
                        i++;
                        ...
                        if(i == p){
                            console.log("资源完全加载")
                            dynamicLoadJs("./mobile/Main.js")
                            LOADING.run(100,"加载完毕");
                        }
                    })
                }
                if(typeof arg[val].skin != 'undefined'){
                    dynamicLoadJs(arg[val].skin,()=>{
                        i++;
                        dynamicLoadJs(arg[val].entity)
                        var p = RESOURCES[0]["script"].length + RESOURCES[1]["image"].length;
                        LOADING.run(i/p*100,"正在加载资源:" + arg[val].path);
                        if(i == p){
                            console.log("资源完全加载")
  ...
  function dynamicLoadJs(url, callback) {
    ...
    if(isPC){
       ...
       dynamicLoadJs("./RES_MOBILE.js",loadResourceMobile);
